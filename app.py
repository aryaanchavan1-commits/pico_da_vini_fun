...
<style>
    ...
    * {
    ...
    /* Remove horizontal lines/separators in content */
    .stMarkdown hr, hr {
    ...
    /* Remove any default borders between sections */
    [data-testid="stVerticalBlock"] {
        border: none !important;
    }
    
    /* Remove black line from home tab sections */
    div[data-testid="stVerticalBlockBorderWrapper"] {
        border: none !important;
        box-shadow: none !important;
    }
    
    /* Target Streamlit divider */
    .stDivider {
        display: none !important;
    }
    
    /* Remove border from profile columns */
...
# Initialize Neon DB (PostgreSQL) connection - cached for performance
@st.cache_resource(ttl=300)
def get_db_connection():
    """Get PostgreSQL connection from Neon DB using Streamlit secrets - cached for 5 minutes"""
    try:
...
# Initialize database - cached to avoid repeated initialization
@st.cache_resource(show_spinner=False)
def init_db():
    """Initialize the database tables - cached"""
    ...
    if conn:
        try:
            ...
            try:
                cur.execute("ALTER TABLE user_inquiries ADD COLUMN IF NOT EXISTS message TEXT")
            except Exception:
                try:
                    cur.execute("ALTER TABLE user_inquiries ADD COLUMN message TEXT")
                except Exception:
                    pass
...
# Load visitor data from database - cached
@st.cache_data(ttl=60)
def load_visitors():
    """Load visitor tracking data from Neon DB - cached for 1 minute"""
    conn = get_db_connection()
    if conn:
        try:
            cur = conn.cursor(cursor_factory=RealDictCursor)
            cur.execute("SELECT * FROM visitors ORDER BY visited_at DESC LIMIT 100")
            visitors_data = cur.fetchall()
            cur.close()
            conn.close()
            return [dict(row) for row in visitors_data] if visitors_data else []
        except Exception:
            return []
    return []

# Track visitor and save to database - optimized
def track_visitor(ip_address, user_agent, device_type, browser, os_name):
    """Save visitor information to the database - runs in background"""
    import threading
    ...
    def _track():
        ...
        if conn:
            try:
                ...
                cur.execute(
                    """INSERT INTO visitors (ip_address, user_agent, device_type, browser, os, visited_at) 
                       VALUES (%s, %s, %s, %s, %s, %s)""",
                    (ip_address, user_agent, device_type, browser, os_name, current_time)
                )
                conn.commit()
                cur.close()
                conn.close()
    ...
    # Run tracking in background thread to not block UI
    thread = threading.Thread(target=_track, daemon=True)
    thread.start()

# Load admin settings from database - cached for performance
@st.cache_data(ttl=30, show_spinner=False)
def load_admin_settings_cached():
    """Load admin settings from Neon DB - cached for 30 seconds"""
    init_db()
...
def load_admin_settings():
    """Load admin settings from database - calls cached version"""
    return load_admin_settings_cached()

# Save admin settings to database - with cache clearing
def save_admin_settings(settings):
    """Save admin settings to Neon DB and clear cache"""
    init_db()
    ...
    if conn:
        try:
            ...
            load_admin_settings_cached.clear()
...
# Initialize database on startup (with error handling) - optimized
try:
    init_db()
except Exception:
    pass

# Track visitor on each visit (using session state to avoid duplicates) - optimized with caching
if 'visitor_tracked' not in st.session_state:
    st.session_state.visitor_tracked = True
    
    # Get IP from server-side (most reliable) - cached
    @st.cache_data(ttl=3600)
    def get_visitor_ip():
        """Get visitor IP address - cached for 1 hour"""
        try:
    ...
    # Get browser/OS info from User-Agent - lightweight
    visitor_browser = "Chrome"
    visitor_os = "Windows"
    visitor_device = "Desktop"
    visitor_ua = "Unknown"
    
    # Track the visitor in the database (runs in background)
...
def get_image_base64(image_path):
    """Convert image to base64 for display - cached for performance"""
    @st.cache_data(ttl=300, show_spinner=False)
    def _get_image_base64(path):
        """Internal cached function to convert image to base64"""
        try:
...
def render_profile_section():
    """Render the main profile section - optimized with session state caching"""
    # Use session state for settings to avoid repeated DB calls
    if 'cached_settings' not in st.session_state:
        st.session_state.cached_settings = get_admin_settings()
    current_settings = st.session_state.cached_settings
    ...
    with col1:
        ...
        # Try to display image with 3D animated border - use cached base64
        if os.path.exists(profile_image_path):
            try:
                # Use cached base64 encoding
...
def render_services_section():
    """Render services section from admin settings - optimized with session state"""
    # Use session state for settings to avoid repeated DB calls
    if 'cached_settings' not in st.session_state:
        st.session_state.cached_settings = get_admin_settings()
    current_settings = st.session_state.cached_settings
...
def render_inquire_section():
    """Render inquiry form section - optimized with session state"""
...
def render_chatbot_section():
    """Render AI chatbot section - optimized with session state"""
    ...
    # Get API key from st.secrets (not from admin panel)
    saved_api_key = st.secrets.get("GROQ_API_KEY", "")
    
    if not saved_api_key:
        st.warning("⚠️ Groq API Key not configured. Please add GROQ_API_KEY in Streamlit Cloud secrets settings.")
        return
...
def render_projects_section():
    """Render projects section with uploaded project images, videos and names"""
    ...
    for i in range(0, len(projects), cols_per_row):
        ...
        for j, col in enumerate(cols):
            ...
            if idx < len(projects):
                ...
                project_video = project.get('video', '')
                
                with col:
                    st.markdown("<div style='text-align: center; margin-bottom: 2rem;'>", unsafe_allow_html=True)
                    
                    # Display project video if available, otherwise show image
                    if project_video:
                        try:
                            # Decode base64 video
                            video_data = base64.b64decode(project_video)
                            # Create video HTML
                            video_html = f"""
                            <video width="100%" controls style="border-radius: 12px; margin-bottom: 0.5rem;">
                                <source src="data:video/mp4;base64,{project_video}" type="video/mp4">
...
def render_admin_panel():
    ...
    with st.expander("🚀 Projects (Image & Name)"):
        ...
        col_proj1, col_proj2, col_proj3 = st.columns([2, 1, 1])
        ...
        with col_proj3:
            new_project_video = st.file_uploader("Project Video (Optional)", type=["mp4", "webm", "mov"], label_visibility="collapsed", key="new_proj_video")
        ...
        if new_project_name and new_project_image:
            if st.button("➕ Add Project", type="primary", width='stretch'):
                ...
                # Read and encode video if provided
                video_base64 = ""
                if new_project_video:
                    video_bytes = new_project_video.read()
                ...
                if video_base64:
                    project_data["video"] = video_base64
    ...
    st.markdown("<h3 style='color: white; font-size: 1.8rem; margin: 3rem 0 1.5rem;'>👥 Visitor Analytics</h3>", unsafe_allow_html=True)
    ...
    if visitors:
        ...
        with col1:
            st.metric("Total Visitors", len(visitors))
        with col2:
            # Count unique IPs
            unique_ips = len(set(v.get('ip_address') for v in visitors if v.get('ip_address')))
            st.metric("Unique IPs", unique_ips)
        with col3:
            # Most common browser
            browsers = [v.get('browser') for v in visitors if v.get('browser')]
            most_common_browser = max(set(browsers), key=browsers.count) if browsers else "N/A"
            st.metric("Top Browser", most_common_browser)
        
        st.markdown("<br>", unsafe_allow_html=True)
        
        # Show visitor table
        for visitor in visitors:
            visited_at = visitor.get('visited_at', '')
            if visited_at:
                try:
            ...
            <div class="admin-section">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        <h4 style="color: white; margin-bottom: 0.5rem;">🌐 {visitor.get('ip_address', 'Unknown IP')}</h4>
                        <p style="color: rgba(255,255,255,0.7); margin: 0.2rem 0;">💻 {visitor.get('browser', 'Unknown')} | {visitor.get('os', 'Unknown OS')}</p>
                        <p style="color: rgba(255,255,255,0.7); margin: 0.2rem 0;">📱 {visitor.get('device_type', 'Desktop')}</p>
                    </div>
                    <div style="text-align: right;">
                        <p style="color: rgba(255,255,255,0.5); font-size: 0.8rem;">🕐 {visited_at}</p>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    else:
        st.info("No visitors tracked yet. Visitors will appear here when they access your site.")
...
def main():
    ...
    with tab1:
        ...
        # About section - use cached settings from session state
        about_text = st.session_state.cached_settings.get('about', '')
        if about_text:
    ...
    # Footer - use cached settings from session state
    footer_name = st.session_state.cached_settings.get('name', 'Aryan Chavan')
