...
def get_db_connection():
    ...
    try:
        ...
        if db_url:
            ...
            # Silent fail - database URL not configured
            return None
    except psycopg2.OperationalError as e:
        # Connection error - silently fall back to JSON
        return None
    except Exception as e:
        # Other errors - silently fall back to JSON
        return None
    ...
    if conn:
        try:
            ...
            try:
                ...
                # Fallback: try without IF NOT EXISTS for older PostgreSQL versions
                try:
...
def load_visitors():
    """Load visitor tracking data from Neon DB"""
    conn = get_db_connection()
    if conn:
        try:
...
def save_admin_settings(settings):
    ...
    if conn:
        try:
            ...
            # Use a more robust UPSERT with proper PostgreSQL syntax
            cur.execute(
...
try:
    ...
    # Silent fail - JSON fallback will handle data storage
    pass
...
if 'visitor_tracked' not in st.session_state:
    ...
    # Use query params to pass data from JavaScript to Python
    import urllib.request
    import json
    from datetime import datetime
    
    # Get IP from server-side (most reliable)
    visitor_ip = "Unknown"
    try:
        with urllib.request.urlopen('https://api.ipify.org?format=json', timeout=3) as response:
            ip_data = json.loads(response.read().decode())
            visitor_ip = ip_data.get('ip', 'Unknown')
    except Exception:
        pass
    
    # Use JavaScript to get browser/OS info and pass via query params
    import streamlit.components.v1 as components
    
    # Create a hidden iframe that will set query params and trigger rerun
    tracking_html = """
    <script>
    (function() {
        const ua = navigator.userAgent;
        ...
        // Encode data
        const data = btoa(JSON.stringify({browser, os, device, ua}));
        
        // Set URL params without reload
        const url = new URL(window.location.href);
        url.searchParams.set('vinfo', data);
    ...
    # Try to read visitor info from query params
    try:
        query_params = st.query_params
        if 'vinfo' in query_params:
            import base64
            vinfo_data = query_params['vinfo']
            vinfo = json.loads(base64.b64decode(vinfo_data).decode())
            visitor_browser = vinfo.get('browser', 'Unknown')
            visitor_os = vinfo.get('os', 'Unknown')
            visitor_device = vinfo.get('device', 'Desktop')
            visitor_ua = vinfo.get('ua', 'Unknown')
        else:
            # Default values - try to detect from User-Agent header
            visitor_browser = "Chrome"  # Most common
            visitor_os = "Windows"
